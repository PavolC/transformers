...
import { exportProgress, importProgress, loadCompleted, resetAll, resetExercise, subscribeProgress } from "../state/progress";
...
export function StartPage({ onGoTo }: { onGoTo: (id: string) => void }) {
	...
	const forgetAll = () => {
		if (window.confirm("Forget everything? Your file, your hints and your passed marks are cleared from this browser. A progress file you saved earlier still loads afterwards.")) {
			resetAll();
	...
	return (
		<article className="module start-page">
			...
			<ol className="start-steps">
				...
				<li>Assemble the real thing: put the pieces together into something recognizably related to the system itself, not just an analogy.</li>
			</ol>

			<h3 id="start-how">How the machinery works</h3>
			<ul className="start-facts">
				<li>
					<b>Python really runs here.</b> An in-page editor sends your code to CPython 3.14 compiled to WebAssembly (Pyodide), with NumPy, in a Web Worker so a training run never freezes the
					page. The first run downloads about 15 MB of runtime and the browser caches it after that.
				</li>
				<li>
					...
					this page, the runtime and the Shakespeare the course trains on.
				</li>
				<li>
				...
				<li>
					<b>Nothing is locked.</b> Every chapter is open from the tabs at any time, in any order. A section you have not written yet is filled in from the course's copy for a run, and the
					panel names what it borrowed; once you have written it, yours is what runs.
				</li>
				<li>
					<b>Each exercise shows its work.</b> The tests are readable in the page, hints come in stages that you choose to open, and the reference solution is the last of them. When a run
      ...
      <details className="notation">
        ...
        {NOTATION.length === 0 ? (
          ...
          <div className="table-scroll scroll-x" tabIndex={0}>
			...
			<p>
				Everything you write in this course goes into one Python file, a section per exercise, in the order you meet them. {passed.length} of {EXERCISES.length}{" "}
				{EXERCISES.length === 1 ? "section is" : "sections are"} passing here.{" "}
				{passed.length === 0 ? "Nothing is saved yet." : "Resetting one puts back its starting text and clears its hints and its passed mark; the rest of the file is untouched."}
			</p>
					...
					return (
						<li key={section.id} className={state === "passing" ? "start-done" : ""}>
							...
							<span className="start-progress-name">
								...
								<span className="start-progress-where">{section.kind === "given" ? (state === "missing" ? "not in your file yet" : "in your file") : STATE_WORDS[state]}</span>
								<span className="start-progress-builds">{exercise ? exercise.builds : section.provides.join(", ")}</span>
								{state === "stale" && <span className="start-progress-warn">Passed once, and the text has changed since. Run its tests again to know where it stands.</span>}
							</span>
			...
			<p className="start-storage-note">
				A saved file holds your whole scribe.py, which hints you opened, and which sections passed. Loading one replaces the sections it names and leaves the rest alone. Downloading scribe.py
				is a different thing: that is the Python itself, ready to run anywhere NumPy is installed, and it is not a file this page can load back.
			</p>
		</article>
	);
}
