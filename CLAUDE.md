...
/public/data/        committed datasets, and bench/ for committed bench output
...
- `npm run bench` regenerates `public/data/bench/*.json` under the pinned Pyodide in
  Node, printing each prose sentence it backs. Needs Node 22 and network for the first
...
- **2026-08-31, benches run under the pinned Pyodide in Node**, so every number the
  ...
  Interactives that show a number the prose quotes replay committed bench JSON; free-play
  interactives compute live and their numbers are never quoted. Rejected: native-Python
