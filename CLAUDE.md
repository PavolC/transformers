...
- To drive the real workbench in a headless browser (the only way to catch a fetch the
  checkers stand in for), build, serve `dist/`, and click through with playwright-core;
  seed `tf:v1:code:workbench` and `tf:v1:code:scratch` in localStorage to skip typing.
